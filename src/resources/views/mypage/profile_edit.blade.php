...
<div class="auth-container">
    ...
    <form class="auth-form" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" novalidate>
        ...
        <div class="avatar-row">
            @php
                $avatarPath = optional($profile)->avatar_path ?? null;
            @endphp

            <img
                class="avatar-preview"
                src="{{ $avatarPath ? asset('storage/' . $avatarPath) : asset('images/avatar-placeholder.png') }}"
                alt="プロフィール画像"
            >
        ...
        <div class="form-group">
            ...
            <input type="text"
                name="name"
                value="{{ old('name', $userNameForForm ?? '') }}"
        ...
        <div class="form-group">
            ...
            <input type="text"
                ...
                class="{{ $errors->has('building') ? 'is-invalid' : '' }}">
            @error('building')
                <div class="form-error">{{ $message }}</div>
            @enderror
        </div>
